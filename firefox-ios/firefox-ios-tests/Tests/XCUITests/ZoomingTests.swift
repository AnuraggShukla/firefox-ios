...
class ZoomingTests: BaseTestCase {
    ...
    func zoomIn() {
        for i in 0...3 {
            ...
            let previoustTextSize = app.staticTexts.elementContainingText("Mission-driven").frame.size.height
            zoomInButton.tap()
            let currentTextSize = app.staticTexts.elementContainingText("Mission-driven").frame.size.height
